...
struct thread_data{
    int x, y;
    thread_data(int a, int b) : x(a), y(b){}
};
bool get_back_to_work = false;
unsigned __stdcall set_pixel(void *data){
    while(!get_back_to_work);
...
[&f](){
    ...
    for(int j = height - 1, k = 0; j >= 0; --j){
        for(int i = 0; i < width; ++i, ++k){
            thread_data a(i, j);
            //h_array[k] = CreateThread(0, 0, set_pixel, &a, 0, 0);
            h_array[k] = (HANDLE)_beginthreadex(0, 0, set_pixel, &a, 0, 0);
        }
    }
    std::cout << "hello\n";
    WaitForMultipleObjects(height * width, h_array, TRUE, INFINITE);
};
std::function<void(int, int)> resize =
[](int width, int height){
    h_array = (HANDLE*)realloc(h_array, sizeof(HANDLE) * width * height);
};
